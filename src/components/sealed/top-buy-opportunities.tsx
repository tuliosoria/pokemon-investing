...
export function TopBuyOpportunities() {
  ...
  const learnMoreOpp = learnMoreId ? opportunities.find((o) => o.id === learnMoreId) : null;
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {opportunities.map((opp, i) => (
          <div
            key={opp.id}
            className="group rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] overflow-hidden hover-lift transition-all flex flex-col"
          >
            <Link href="/sealed-forecast" className="block">
              {/* Rank + image header */}
            ...
            {/* Learn more button — separate from the Link */}
            {opp.set && opp.forecast && (
              <div className="px-4 pb-3 flex justify-end">
                <button
                  type="button"
                  onClick={() => setLearnMoreId(opp.id)}
                  aria-label={`Learn more about ${opp.name}`}
      ...
      {/* Breakdown modal */}
      {learnMoreOpp?.set && learnMoreOpp?.forecast && (
        <ForecastBreakdownModal
          set={learnMoreOpp.set}
          forecast={learnMoreOpp.forecast}
